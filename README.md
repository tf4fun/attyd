...
- one isolated ACP upstream connection per browser connection; stdio mode launches one Agent process for that connection;
...
npm test
npm run test:rust
npm run test:rust:remote
npm run test:coverage:backends
npm run test:goose
npm run test:ui
...
`npm test` covers ACP stream reduction, Zed-compatible assistant/tool/plan behavior, prompt stop/usage validation, Agent-owned authenticate/logout negotiation, real-PTY terminal-auth launch/input/resize/cancel/reconnect, and `auth_required` recovery, cross-session caching and elicitation-scope isolation, exact request-scoped acknowledgement matching (including self-contained permission/elicitation response locks, stale acknowledgements, local failure display, and retry), rapid duplicate prompts, out-of-order and pre-response session creation/fork updates, transactional session-switch/fork/close rollback, safe rejected-session allocation cleanup, delayed fork/close/control races with failure recovery, complete startup/subprocess terminal-state cleanup, bidirectional delete/attach exclusion, browser transport and recoverable-error state, bounded structured ACP JSON-RPC error details with exact failed-ContentBlock retry state, complete response/notification relay budgets including outer `_meta`, bounded browser identifiers, runtime server-event envelopes, optional-method capability gates, browser/Agent RPC cancellation across filesystem reads, terminal waiters, interactions, and MCP connect/messages, bounded form and URL flow validation, full representable request-scope IDs, schema-valid elicitation defaults, semantic component markup, stacked interaction focus restoration, keyboard cancellation and slash-command selection/submission, complete-ContentBlock prompt-history recall, bounded semantic thread search and DOM visibility rules, loaded-session fuzzy filtering and chronological grouping, mobile-drawer focus and Escape behavior, bounded pagination with duplicate-ID upserts, Agent-owned session lifecycle and fork forwarding, static session setup, portable remote paths, bounded filesystem and terminal services, unadvertised NES/document parser hardening, transactional cumulative session-update and Agent NDJSON bounds, MCP-over-ACP request/notification routing and connection isolation, deterministic adversarial inputs, and fake-agent bridge round trips. `npm run test:goose` launches the bundled real Goose with isolated temporary config/data/state directories. It verifies ACP v1 initialization, the expected unconfigured-provider `session/new` error, and subsequent `session/list` recovery. It then uses a deterministic loopback-only OpenAI-compatible fixture—never an external model endpoint or real API key—to exercise `session/new` → `session/prompt` → developer shell `tool_call`/`tool_call_update` → Agent message → `end_turn` through both the raw SDK and the complete attyd bridge/event-parser/browser-reducer path, including ACP terminal output, exit, and release presentation when the Client advertises terminal support.

`npm run test:ui` starts the Rust binary on an ephemeral loopback port, serves its embedded production bundle, and replays every real bridge event through the production browser reducer. It verifies pre-response session creation/fork replay, Agent-owned history cwd and configuration, semantic prompt stop/usage, structured request failures, all ACP content-block variants, live thought/tool/answer activity, read-only aggregation of Agent-reported diffs, live terminal-backed tool calls with output/exit/release presentation and cancelled-wait recovery, cancellable filesystem reads/pre-mutation writes, form elicitation, URL consent/completion/session-close abortion, transactional session fork/close/delete, cached background-session isolation, context compaction, bounded incomplete Agent stdout, and complete browser-bridge → fake Agent → ACP-transport MCP round trips including message cancellation, disconnect recovery, and secret non-disclosure. `npm run test:ui:node` runs the same black-box contract against the migration oracle. For the interactive fixture, run `npm run test:ui:serve -- 7332`, open `http://127.0.0.1:7332`, and submit `structured-error-flow`, `usage-flow`, `context-window-flow`, `content-flow`, `activity-flow`, `message-actions-flow`, `review-flow`, `terminal-flow`, `terminal-cancel-flow`, `terminal-burst-flow`, `filesystem-cancel-flow`, `form-flow`, `url-flow`, `pending-url-flow`, `background-flow`, `compaction-flow`, `mcp-flow`, or `mcp-cancel-flow`; type `/` for Agent commands or `@` to search workspace text files. The sidebar's MCP-over-ACP inspector shows the initialize/echo/notification/reverse-request lifecycle; provider command and environment values must not appear in browser state.

`npm run test:browser` launches the same production fixture in real Chromium through Playwright. It drives permission and form ACP interactions, verifies the permission subject exposes the live tool's name, location, and default-visible raw input before approval, checks first-action focus plus composer restoration, exercises queued follow-ups including edit and Send now cancellation followed by FIFO dispatch, verifies `auth_required` first-action focus followed by Agent-handled authentication and real-PTY terminal authentication/reconnect, restored session history, capability-gated logout, and reauthentication without client-defined credential fields, sends clipboard-image, dropped-file, and `@`-selected workspace context through the real Agent prompt, recalls the complete attachment prompt through Zed-style ↑/↓ history, expands the read-only Agent diff review without editor actions, verifies live thinking plus textual tool lifecycle/default rich-content disclosure, inspects structured ACP error data and retries the exact failed prompt without exposing an editor surface, exercises the mouse/keyboard Agent-response context menu plus Markdown export, searches visible semantic Agent-thread content with match navigation/highlighting and regex error handling, verifies completed ACP compaction disclosure/search behavior, renders live Agent-reported context usage/cost beside the composer with keyboard dismissal, toggles the same ACP composer into a Zed-style expanded message composer on desktop/mobile and confirms a pending permission remains visible, validates long-thread navigation, exercises mobile sidebar Escape/focus behavior, rejects browser console/page errors, and detects desktop/mobile horizontal layout overflow. Local runs use an installed Chrome; CI installs Playwright Chromium and retains traces, screenshots, and an HTML report on failure. The GitHub Actions workflow runs type/unit/build, bridge UI smoke, and this rendering-engine layer from a clean `npm ci` install.
