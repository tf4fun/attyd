...
- capability-gated `session/list`, `session/load`, `session/resume`, `session/close`, and `session/delete`, with bounded pagination and the Agent remaining the only persistent completed-history authority; cold observation loads once, while an open session uses its in-memory baseline plus active turn and never reloads after a prompt; if a terminal turn is committed with no session SSE subscriber, attyd closes and releases that session when the Agent supports close, so its next observation follows the cold-load path; discovery omits the cwd filter on every transport, and every Agent-returned session retains its own portable absolute cwd; mobile resume performs a bounded browser-bridge ping/pong probe and a terminal connection state exposes an explicit reconnect action over the disabled composer;
...
The homepage (`/`) shows projects grouped by working directory. **New project**
asks for a working directory and creates the project's first session. Open
`/projects/<encoded-cwd>` to browse that project's sessions, then
`/projects/<encoded-cwd>/sessions/<sessionId>` to open a conversation. Pages use the
full width, with navigation, titles, and actions integrated into each page.
The session title dropdown switches within the current project; Agent settings
sit alongside the page actions. A separate back button takes a session to its
project and a project to the homepage, including on directly opened links.
Breadcrumbs, refresh,
and browser back/forward preserve this hierarchy. Legacy `/sessions/<sessionId>`
links resolve to the Agent's actual project. A session that no longer exists
...
New thread asks for an absolute Agent workspace. From a project it pre-fills that project's cwd; from the homepage it uses `--cwd` (or attyd's startup directory) for stdio and starts blank for HTTP/WS. Session discovery does not filter by the startup cwd. On every transport, `session/list` results retain their own cwd, which load/resume/fork continue to use. Projects and their sessions are ordered by recent activity. Project and session pages discover all metadata pages; the homepage offers Load more. Search covers loaded paths and thread metadata. Navigating out of a conversation preserves running work and its in-memory state.
...
Rust host (single executable)
  ...
  ├─ in-memory baseline + one active turn, keyed by sessionId
...
The Agent remains the only persistent completed-history authority. The Rust Bridge keeps an
