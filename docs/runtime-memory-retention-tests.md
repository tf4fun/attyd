...
最初 Red 基线：`0857250`（历史 workflow 生命周期修复），测试提交为 `40e1c0a`。
2026-09-22 已拉取生产修复 `802c20b`，既有 35 项 Rust 内存门槛和 10 项浏览器门槛全部通过。
下文保留修复前的诊断、覆盖范围和 Red 数量；第二阶段的新门槛、实现和全量 Green 结果见
[内存效率方案](runtime-memory-efficiency.md)。
...
REST 最新视图可能先于 SSE 失败事件安装。修复前 hook 把较旧 revision 的失败事件当作
...
第一阶段要求所有 Red 转 Green；这些门槛现已通过，后续须继续保持。
完整优化还需满足第二阶段门槛与实机验证，不能把第一阶段 Green 解释为所有成本已消除。
