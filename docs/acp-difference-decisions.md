...
| T06 / 卡片与媒体 | 必要字段语义和内容必须正确；不规定布局、折叠、编辑器导航或所有二进制播放器 | 多种编辑器呈现方式；本版本 audio/混合媒体仍有降级 | 统一卡片、原始细节折叠、Web media；binary resource 暂为摘要 | 2026-09-26 更新：附件统一为紧凑入口，工具内采用文件行；点击在新页面打开，预览/下载交由浏览器。内容由 attyd 会话内存托管，不新增预览器或持久化。 |
...
2026-09-26 更新：以下策略覆盖此前的内嵌预览/附件下载方案。第五、六节的测试结果仍为原轮次的历史记录；当前展示检查见 [专项对照报告](acp-display-audit-2026-09-26.md)。
...
| 图片、音频、文本及二进制资源 | 使用统一紧凑附件入口；工具输出采用文件行。显示名称及可用大小，类型/地址/description 放入提示信息；不在消息中展开附件正文或播放器。 |
| 打开方式 | 单一链接在浏览器新页面打开，浏览器与响应 MIME 决定预览或下载；不提供两个模式按钮，不引入 PDF、Office 等专用预览器。 |
| 仅有资源链接 | 展示名称和地址，HTTP(S) 或 attyd 已托管的引用由用户点击打开；只有 file/custom URI 时保留描述，不猜内容、不自动读取宿主机路径或获取远程数据。 |
...
附件数据来源遵循 T03：Agent 提供的内容优先，其次当前内存缓存，再明确缺失。普通浏览器会话视图只传附件引用；打开时从 attyd 已有会话内存读取，不新增附件仓库或前端持久化缓存。会话释放/进程重启后，旧引用不再可用；冷恢复取决于 Agent 是否重新提供内容。

本项已实施：普通与内嵌 image/audio、文本及 binary resource 使用同一打开逻辑；文本响应使用 UTF-8，二进制保留内容类型。URL consent、取消状态及字段合并正确性继续属于必要行为，不因展示取舍降级。
